...
The full-site source uses uncapped discovery (max_urls: 0). A 60-minute GitHub Actions job timeout is used as an operational guardrail.
...
Homepage -> URL discovery -> canonicalization -> concurrent fetch -> HTML/PDF/DOCX/XLSX/PPTX/CSV/text extraction -> deterministic normalization -> normalized SHA-256 -> N vs N-1 event detection -> immutable evidence/diff -> high-recall relevance -> structured AI analysis -> ntfy -> historical state -> live console.
...
It can also be started manually with source selection and a controlled notification test. A concurrency lock prevents overlapping state writers.
